...
function parseProducts(rows: Record<string, unknown>[]) {
  return rows
    ...
    .map(r => ({
      ...
      description: String(r.description || r.Description || r.descripcion || r.Descripcion || "") || undefined,
      ...
      brand: String(r.brand || r.Brand || r.marca || r.Marca || "") || undefined,
      stock: parseInt(String(r.stock || r.Stock || r.existencia || "0"), 10) || 0,
      imageUrl: String(r.imageUrl || r.image_url || r.imagen || r.image || r.foto || "") || undefined,
      isActive: true,
    }))
...
router.post("/products/import/excel", upload.single("file"), async (req, res) => {
  ...
  try {
    ...
    const isCsv = fileName.endsWith(".csv") || req.file.mimetype === "text/csv" || req.file.mimetype === "text/plain";
    ...
    if (isCsv) {
      ...
      if (lines.length < 2) { res.status(400).json({ error: "El CSV está vacío o no tiene datos" }); return; }
      ...
      if (!wb.SheetNames.length) { res.status(400).json({ error: "El archivo Excel no tiene hojas" }); return; }
    ...
    if (!products.length) { 
      const sample = rows[0] ? Object.keys(rows[0]).join(", ") : "sin columnas";
      res.status(400).json({ 
...
router.post("/products/import/json", async (req, res) => {
  ...
  if (!Array.isArray(rawProducts) || !rawProducts.length) {
    res.status(400).json({ error: "Se esperaba un objeto { products: [...] } con al menos un producto" }); return;
  }
  try {
    const products = parseProducts(rawProducts);
    if (!products.length) { res.status(400).json({ error: "Ningún producto tiene los campos requeridos: name/nombre, price/precio, category/categoria" }); return; }
    const inserted = await db.insert(productsTable).values(products).returning();
...
router.post("/products/import/woocommerce", async (req, res) => {
  ...
  if (!url || !consumerKey || !consumerSecret) {
    res.status(400).json({ error: "url, consumerKey y consumerSecret son requeridos" }); return;
  }
  try {
    ...
    if (!response.ok) {
      ...
      res.status(400).json({ error: `WooCommerce respondió con error ${response.status}: ${errText.substring(0, 300)}` }); 
      return;
    }
    ...
    if (!Array.isArray(wcProducts)) {
      res.status(400).json({ error: "La respuesta de WooCommerce no es un array. Verifica la URL y las credenciales." });
      return;
    }
    ...
    if (!products.length) { 
      res.status(400).json({ error: `WooCommerce devolvió ${wcProducts.length} productos pero ninguno tiene nombre y precio válidos` }); 
      return; 
    }
    ...
    res.status(500).json({ error: `Error conectando con WooCommerce: ${err.message}` });
  }
});
